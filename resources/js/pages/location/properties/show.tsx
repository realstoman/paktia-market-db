...
import { Property, BreadcrumbItem, PropertyFloor } from '@/types';
...
export default function PropertyShow({ property }: { property: Property }) {
    ...
    const isMarket = ['market', 'mall'].includes(property.property_type ?? 'market');
    const breadcrumbs: BreadcrumbItem[] = [{ title: 'Markets & Properties', href: '/properties' }, { title: property.name, href: `/properties/${property.id}` }];
    ...
    return <AppLayout breadcrumbs={breadcrumbs}>
        ...
        <div className="space-y-6">
            <section className="relative min-h-64 overflow-hidden rounded-2xl bg-gradient-to-r from-emerald-950 to-teal-700">
                {property.image_url && <img src={property.image_url} alt="" className="absolute inset-0 h-full w-full object-cover opacity-45" />}
                <div className="relative flex min-h-64 flex-col justify-end bg-gradient-to-t from-black/80 to-transparent p-6 text-white md:p-8">
                    <div className="mb-3 flex gap-2"><Badge className="bg-white/90 text-slate-900">{labelType(property.property_type)}</Badge><Badge variant="outline" className="border-white/50 text-white">{property.usage_type}</Badge></div>
                    <h1 className="text-3xl font-bold">{property.name}</h1><p className="mt-2 flex items-center gap-2 text-white/80"><MapPin className="h-4 w-4" />{property.address || 'Address not specified'}</p>
                </div>
            </section>

            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <Metric icon={Layers3} label="Configured floors" value={floors.length} hint={property.declared_floors ? `${property.declared_floors} planned` : undefined} />
                <Metric icon={isMarket ? Store : DoorOpen} label={isMarket ? 'Shops' : property.property_type === 'block' ? 'Apartments' : 'Rooms'} value={property.property_type === 'house' ? (property.rooms_count ?? 0) : units.length} hint={property.declared_units ? `${property.declared_units} planned` : undefined} />
                <Metric icon={Ruler} label="Building area" value={property.building_area_sqm ? `${property.building_area_sqm} m²` : '—'} />
            ...
            <div className="grid gap-6 xl:grid-cols-[1fr_2fr]">
                <Card><CardHeader><CardTitle>Property profile</CardTitle></CardHeader><CardContent className="space-y-3 text-sm"><Detail label="Country / Province" value={`${resolveName(property.country)} / ${resolveName(property.province)}`} /><Detail label="Land area" value={property.land_area_sqm ? `${property.land_area_sqm} m²` : '—'} /><Detail label="Distance from city" value={property.distance_from_city_km ? `${property.distance_from_city_km} km` : '—'} /><Detail label="Parking" value={property.parking_spaces ?? '—'} />{property.property_type === 'house' && <><Detail label="Rooms" value={property.rooms_count ?? '—'} /><Detail label="Kitchens / halls" value={`${property.kitchens_count ?? 0} / ${property.halls_count ?? 0}`} /><Detail label="Bathrooms" value={property.bathrooms_count ?? '—'} /></>}<div className="pt-2"><p className="font-medium">Description</p><p className="mt-1 text-muted-foreground">{property.description || 'No description recorded.'}</p></div></CardContent></Card>
                <div className="space-y-4">
                    <div className="flex items-center justify-between"><div><h2 className="text-xl font-semibold">Floors & {isMarket ? 'shops' : 'apartments'}</h2><p className="text-sm text-muted-foreground">Basements use a negative floor number (for example, -1).</p></div>{property.property_type !== 'house' && <AddFloor property={property} />}</div>
                    {property.property_type === 'house' ? <Card><CardContent className="py-10 text-center text-muted-foreground"><Building2 className="mx-auto mb-3 h-10 w-10" />House details are managed at property level. Its complete house can later be assigned to one rental contract.</CardContent></Card> : floors.length ? floors.map((floor) => <FloorCard key={floor.id} property={property} floor={floor} isMarket={isMarket} />) : <div className="rounded-xl border border-dashed py-16 text-center text-muted-foreground">No floors yet. Add the basement, ground floor, or upper floors to begin.</div>}
                </div>
            </div>
        </div>
    </AppLayout>;
...
function FloorCard({ property, floor, isMarket }: { property: Property; floor: PropertyFloor; isMarket: boolean }) {
    return <Card><CardHeader className="flex-row items-start justify-between space-y-0"><div><CardTitle>{floor.name}</CardTitle><p className="mt-1 text-sm text-muted-foreground">Level {floor.level_number} · {floor.area_sqm ? `${floor.area_sqm} m²` : 'Area not recorded'} · {(floor.units ?? []).length} {isMarket ? 'shops' : 'apartments'}</p></div><div className="flex gap-1"><AddUnit property={property} floor={floor} isMarket={isMarket} /><Button size="icon" variant="ghost" aria-label="Delete floor" onClick={() => confirm('Delete this floor and all of its spaces?') && router.delete(`/properties/${property.id}/floors/${floor.id}`)}><Trash2 className="h-4 w-4 text-destructive" /></Button></div></CardHeader><CardContent>{floor.units?.length ? <div className="grid gap-3 md:grid-cols-2">{floor.units.map((unit) => <div key={unit.id} className="flex items-center justify-between rounded-lg border p-3"><div><div className="flex items-center gap-2 font-medium"><Store className="h-4 w-4" />{isMarket ? 'Shop' : 'Apartment'} {unit.unit_number}<Badge variant={unit.occupancy_status === 'vacant' ? 'success' : 'secondary'}>{unit.occupancy_status}</Badge></div><p className="mt-1 text-xs text-muted-foreground">{unit.area_sqm ? `${unit.area_sqm} m²` : 'No area'}{!isMarket && ` · ${unit.rooms_count ?? 0} rooms · ${unit.bathrooms_count ?? 0} bathrooms`}</p></div><Button size="icon" variant="ghost" onClick={() => confirm('Delete this space?') && router.delete(`/properties/${property.id}/floors/${floor.id}/units/${unit.id}`)}><Trash2 className="h-4 w-4 text-destructive" /></Button></div>)}</div> : <p className="rounded-lg bg-muted/50 py-8 text-center text-sm text-muted-foreground">No {isMarket ? 'shops' : 'apartments'} have been added.</p>}</CardContent></Card>;
}
function AddFloor({ property }: { property: Property }) {
    ...
    const submit = (e: FormEvent) => { e.preventDefault(); form.post(`/properties/${property.id}/floors`, { onSuccess: () => { form.reset(); setOpen(false); } }); };
...
function AddUnit({ property, floor, isMarket }: { property: Property; floor: PropertyFloor; isMarket: boolean }) {
    ...
    const submit = (e: FormEvent) => { e.preventDefault(); form.post(`/properties/${property.id}/floors/${floor.id}/units`, { onSuccess: () => { form.reset(); setOpen(false); } }); };
...
function resolveName(value: Property['country'] | Property['province']) { return typeof value === 'string' ? value : value?.name ?? '—'; }
