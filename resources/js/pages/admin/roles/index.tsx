...
export default function RolesPage({ roles, permissions }: RolesPageProps) {
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        {
            title: t('navigation.roles', 'Roles'),
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={t('roles.page.metaTitle', 'Roles')} />
            <div className="space-y-4">
                <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
                    <SummaryMetricCard
                        title={t('roles.cards.total.title', 'Total Roles')}
                        value={formatNumber(roles.length)}
                        description={t(
                            'roles.cards.total.description',
                            'Role definitions available in the system.',
                        )}
                        icon={ShieldCheck}
                        variant="rose"
                    />
                    <SummaryMetricCard
                        title={t('roles.cards.permissions.title', 'Permissions')}
                        value={formatNumber(permissions.length)}
                        description={t(
                            'roles.cards.permissions.description',
                            'Permission entries available for assignment.',
                        )}
                        icon={LockKeyhole}
                        variant="rose"
                    />
                    <SummaryMetricCard
                        title={t(
                            'roles.cards.configured.title',
                            'Configured Roles',
                        )}
                        value={formatNumber(rolesWithPermissions)}
                        description={t(
                            'roles.cards.configured.description',
                            'Roles that already have permission mappings.',
                        )}
                        icon={UserRoundCog}
                        variant="rose"
                    />
                    <SummaryMetricCard
                        title={t(
                            'roles.cards.averagePermissions.title',
                            'Average Permissions',
                        )}
                        value={
                            roles.length > 0
                        ...
                        description={t(
                            'roles.cards.averagePermissions.description',
                            'Average permission count assigned per role.',
                        )}
                        icon={KeyRound}
                        variant="rose"
                    />
