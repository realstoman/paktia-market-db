...
test('api v1 products show returns a single product', function () {
    ...
    getJson('/api/v1/products/'.$product->id)
        ...
        ->assertJsonPath('data.product_type_pashto_name', 'څښاک')
        ->assertJsonPath('data.product_type_dari_name', 'نوشیدنی')
...
test('api v1 product cuisines index, show, and products endpoint work', function () {
    ...
    $this->getJson('/api/v1/products/cuisines/'.$cuisine->id.'/products')
        ->assertOk()
        ->assertJsonCount(1, 'data')
        ->assertJsonPath('data.0.id', $product->id)
...
test('api v1 product types index and show work', function () {
    ...
    $this->getJson('/api/v1/products/types')
        ->assertOk()
        ->assertJsonPath('data.0.type', 'beverage')
        ->assertJsonPath('data.0.type_pashto_name', 'څښاک')
        ->assertJsonPath('data.0.type_dari_name', 'نوشیدنی')
    ...
    $this->getJson('/api/v1/products/types/'.$type->id)
        ...
        ->assertJsonPath('data.type', 'beverage')
        ->assertJsonPath('data.type_pashto_name', 'څښاک')
        ->assertJsonPath('data.type_dari_name', 'نوشیدنی')
